  ...
  import { Client, GatewayIntentBits, Events, REST, Routes, AttachmentBuilder, SlashCommandBuilder } from 'discord.js';
  ...
  (async () => {
    try {
      ...
      await rest.put(
        ...
        { body: [...commands, ...tldrCommand] } // merge existing commands + tldr
      );
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (interaction.commandName === "tldr") {
    ...
    try {
      ...
      // Fetch last 50 messages (adjust if needed)
      const fetchedMessages = await interaction.channel.messages.fetch({ limit: 150 });
